...
layout(std430, buffer_reference) readonly buffer Positions {
    vec4 pos[]; // pos: xyz, initialAngle: w
};

layout(push_constant) uniform CubePerFrameData {
    mat4 viewproj;
    uint textureId;
    Positions bufId;
    float time;
...
// Cube indices for 6 faces, 2 triangles per face = 36 indices
...
void main() {
    ...
    vec3 xyz = vec3(idx & 1U, (idx & 4U) >> 2, (idx & 2U) >> 1);
