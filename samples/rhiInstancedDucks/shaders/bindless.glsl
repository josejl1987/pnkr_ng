...
vec4 textureBindless2D(uint textureIndex, uint samplerIndex, vec2 uv) {
    return texture(
        sampler2D(
            nonuniformEXT(bindlessTextures[textureIndex]),
            nonuniformEXT(bindlessSamplers[samplerIndex])
        ),
        uv
    );
}
...
vec4 textureBindless2D(uint textureIndex, vec2 uv) {
    return textureBindless2D(textureIndex, kDefaultSamplerIndex, uv);
}
