...
public:
  ComputeTextureApp()
      : app::Application({.title = "RHI Compute Texture",
                          .width = 1280,
                          .height = 720,
  ...
  void onInit() override {
    ...
    textureDesc.usage = renderer::rhi::TextureUsage::Storage |
    ...
    m_sampler = m_renderer->device()->createSampler(
        renderer::rhi::Filter::Linear, renderer::rhi::Filter::Linear,
...

    {
      ...
      auto cs = renderer::rhi::Shader::load(
          renderer::rhi::ShaderStage::Compute,
          getShaderPath("industrial.comp.spv"), reflect);
      auto builder = renderer::rhi::RHIPipelineBuilder()
                         .setComputeShader(cs.get())
                         .setName("IndustrialCompute");
      m_computePipeline =
          m_renderer->createComputePipeline(builder.buildCompute());
    }

    {
      ...
      auto vs = renderer::rhi::Shader::load(
          ...
          getShaderPath("fullscreen.vert.spv"), reflect);
      auto fs = renderer::rhi::Shader::load(
          renderer::rhi::ShaderStage::Fragment,
          getShaderPath("fullscreen.frag.spv"), reflect);
    ...
    m_renderer->setRecordFunc(
        [this](const renderer::RHIFrameContext &ctx) { recordGraphics(ctx); });
  }
  ...
  void recordCompute(const renderer::RHIFrameContext &ctx) {
...

    {
      ...
      cmd->pipelineBarrier(renderer::rhi::ShaderStage::Fragment,
                           renderer::rhi::ShaderStage::Compute, {barrier});
    }
...

    {
      ...
      cmd->pipelineBarrier(renderer::rhi::ShaderStage::Compute,
                           renderer::rhi::ShaderStage::Fragment, {barrier});
    }
  }
