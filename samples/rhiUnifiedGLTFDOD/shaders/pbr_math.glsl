...
PBRInfo calculatePBRInputsMetallicRoughness(InputAttributes tc, vec4 albedo, vec4 mrSample, MetallicRoughnessDataGPU mat) {
    ...
    pbrInputs.attenuation = vec4(1.0, 1.0, 1.0, 0.0); // rgb=color(white), w=distance(0=infinite)
...
vec3 getTransmissionSample(MetallicRoughnessDataGPU mat, vec2 fragCoord, float roughness, float ior) {
    uint textureid = perFrame.drawable.transmissionFramebuffer;
    uint samplerid = perFrame.drawable.transmissionFramebufferSampler;
    const ivec2 size = textureBindlessSize2D(textureid);
    const vec2 uv = fragCoord;
    float framebufferLod = log2(float(size.x)) * applyIorToRoughness(roughness, ior);
    vec3 transmittedLight = textureBindless2DLod(textureid, samplerid, uv, framebufferLod).rgb;
    return transmittedLight;
}

vec3 getIBLVolumeRefraction(MetallicRoughnessDataGPU mat, vec3 n, vec3 v, float perceptualRoughness, vec3 baseColor, vec3 f0, vec3 f90,
vec3 position, mat4 modelMatrix, mat4 viewProjMatrix, float ior, float thickness, vec3 attenuationColor, float attenuationDistance)
{
    ...
    vec3 transmittedLight = getTransmissionSample(mat, refractionCoords, perceptualRoughness, ior);
...
vec3 getLightIntensity(LightDataGPU light, vec3 pointToLight) {
