...
public:
    ...
    void uploadMaterials() {
        ...
        for (const auto& mat : modelMaterials) {
            ...
            if (mat.m_baseColorTexture != INVALID_TEXTURE_HANDLE) {
                auto handle = m_renderer->device()->registerBindlessTexture(m_renderer->getTexture(mat.m_baseColorTexture), m_dummySampler.get());
    ...
    void createPipeline() {
        ...
        auto vs = renderer::rhi::Shader::load(renderer::rhi::ShaderStage::Vertex, getShaderPath("vertex_pulling.vert.spv"), config);
