  ...
  std::visit(
      fastgltf::visitor{
          [&](const fastgltf::sources::Array &a) {
            ...
            if (!bytes.empty()) {
              std::memcpy(bytes.data(), a.bytes.data(), bytes.size());
            }
          },
          ...
          [&](const fastgltf::sources::Vector &v) {
            ...
            if (!bytes.empty()) {
              std::memcpy(bytes.data(), v.bytes.data(), bytes.size());
            }
          },
          ...
          [&](const fastgltf::sources::ByteView &b) {
            ...
            if (!bytes.empty()) {
              std::memcpy(bytes.data(), b.bytes.data(), bytes.size());
            }
          },
          ...
          [&](const fastgltf::sources::BufferView &viewSrc) {
            ...
            std::visit(
                fastgltf::visitor{
                    [&](const fastgltf::sources::Array &a) {
                      copySlice(reinterpret_cast<const std::uint8_t *>(
                                    a.bytes.data()),
                                a.bytes.size());
                    },
                    [&](const fastgltf::sources::Vector &v) {
          ...
          [&](const fastgltf::sources::URI &uriSrc) {
            ...
            } else if constexpr (requires { uriSrc.uri.path(); }) {
              p = (baseDir / std::filesystem::path(uriSrc.uri.path()))
                      .lexically_normal();
            }
      ...
      image.data);
  if (bytes.empty()) {
    core::Logger::Scene.warn("extractImageBytes: returned 0 bytes. mime='{}'",
                             image.name);
    std::visit(fastgltf::visitor{
                   ...
                   [&](const fastgltf::sources::URI &u) {
                     if constexpr (requires {
                       ...
                       core::Logger::Scene.warn(
                           "  source=URI scheme='{}' path='{}'", u.uri.scheme(),
                           std::string(u.uri.path()));
                     } else {
                       core::Logger::Scene.warn(
                   ...
                   [&](auto &) { core::Logger::Scene.warn("  source=other"); }},
               image.data);
  }
  return bytes;
}
...
Transform toTransform(const fastgltf::Node &node) {
  ...
  std::visit(fastgltf::visitor{
                 ...
                 [&](const fastgltf::math::fmat4x4 &m) {
                   ...
                   glm::decompose(out.m_matrix, out.m_scale, out.m_rotation,
                                  out.m_translation, skew, perspective);
                 }},
             node.transform);
  return out;
}
...
void loadSkins(const fastgltf::Asset &gltf, std::vector<Skin> &outSkins,
  ...
  for (const auto &gSkin : gltf.skins) {
    ...
    skin.skeletonRootNode =
        gSkin.skeleton.has_value()
    ...
    if (gSkin.inverseBindMatrices.has_value()) {
      ...
      fastgltf::iterateAccessorWithIndex<glm::mat4>(
          gltf, acc,
          [&](glm::mat4 m, size_t idx) { skin.inverseBindMatrices[idx] = m; });
    } else {
  ...
  for (const auto &gAnim : gltf.animations) {
    ...
    for (const auto &gSampler : gAnim.samplers) {
      ...
      fastgltf::iterateAccessorWithIndex<float>(
          gltf, inputAcc, [&](float t, size_t idx) {
      ...
      if (outputAcc.type == fastgltf::AccessorType::Scalar) {
        fastgltf::iterateAccessorWithIndex<float>(
            gltf, outputAcc, [&](float v, size_t idx) {
        ...
        fastgltf::iterateAccessorWithIndex<glm::vec4>(
            gltf, outputAcc,
            [&](glm::vec4 v, size_t idx) { sampler.outputs[idx] = v; });
      }
...
uint64_t hashImageBytes(const fastgltf::Asset &gltf,
  ...
  std::visit(
      fastgltf::visitor{
          [&](const fastgltf::sources::Array &a) {
            hash = XXH3_64bits(a.bytes.data(), a.bytes.size());
          },
          [&](const fastgltf::sources::Vector &v) {
            hash = XXH3_64bits(v.bytes.data(), v.bytes.size());
          },
          [&](const fastgltf::sources::ByteView &b) {
            hash = XXH3_64bits(b.bytes.data(), b.bytes.size());
          },
          [&](const fastgltf::sources::BufferView &viewSrc) {
            const auto &bv = gltf.bufferViews[viewSrc.bufferViewIndex];
            const auto &buf = gltf.buffers[bv.bufferIndex];
            auto hashSlice = [&](const std::uint8_t *src, size_t srcSize) {
              const size_t off = bv.byteOffset;
              const size_t len = bv.byteLength;
              if (off + len > srcSize) {
                return;
              }
              hash = XXH3_64bits(src + off, len);
            };
            std::visit(
                fastgltf::visitor{
                    [&](const fastgltf::sources::Array &a) {
                      hashSlice(reinterpret_cast<const std::uint8_t *>(
                                    a.bytes.data()),
                                a.bytes.size());
                    },
                    [&](const fastgltf::sources::Vector &v) {
                    ...
                    [&](const fastgltf::sources::URI &u) {
                      ...
                      std::vector<std::uint8_t> whole;
                      if (!p.empty() && readFileBytes(p, whole)) {
                        hashSlice(whole.data(), whole.size());
                      }
                    },
                    [&](auto &) {}},
                buf.data);
          ...
          [&](const fastgltf::sources::URI &uriSrc) {
            if constexpr (requires { uriSrc.uri.scheme(); }) {
              if (uriSrc.uri.scheme() == "data") {
                ...
                const auto comma = s.find(',');
                if (comma != std::string_view::npos) {
                  auto decoded = base64Decode(s.substr(comma + 1));
                  hash = XXH3_64bits(decoded.data(), decoded.size());
                }
                return;
              }
            }
            ...
            if (!p.empty()) {
              std::vector<std::uint8_t> whole;
              if (readFileBytes(p, whole)) {
                hash = XXH3_64bits(whole.data(), whole.size());
              }
            }
          },
          [&](const fastgltf::sources::Fallback &) {}, [&](auto &) {}},
      image.data);
  return hash;
}
} // namespace pnkr::renderer::scene
