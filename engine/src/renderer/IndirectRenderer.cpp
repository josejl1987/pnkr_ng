...
AUTO_CVAR_FLOAT(rShadowBias, "Constant depth bias for shadow mapping", 0.005F);
AUTO_CVAR_INT(r_msaa, "MSAA sample count (1, 2, 4)", 1, core::CVarFlags::save);
...
void IndirectRenderer::init(RHIRenderer *renderer,
                            std::shared_ptr<ModelDOD> model, TextureHandle brdf,
                            TextureHandle irradiance, TextureHandle prefilter,
  ...
  if ((m_resources.irradianceMap == INVALID_TEXTURE_HANDLE ||
       m_resources.prefilterMap == INVALID_TEXTURE_HANDLE) &&
      isCubemap(skyboxCandidate)) {
    core::Logger::Render.info(
        "IndirectRenderer: IBL maps missing, generating from skybox...");
    EnvironmentProcessor processor(m_renderer);
    GeneratedIBL ibl =
        processor.processEnvironment(skyboxCandidate, m_skyboxFlipY);

    if (ibl.irradianceMap == INVALID_TEXTURE_HANDLE ||
        ibl.prefilteredMap == INVALID_TEXTURE_HANDLE) {
      core::Logger::Render.error(
          "IndirectRenderer: Failed to generate IBL maps");
    } else {
      if (m_resources.irradianceMap == INVALID_TEXTURE_HANDLE) {
  ...
  auto comp =
      rhi::Shader::load(rhi::ShaderStage::Compute, "shaders/skinning.spv");
  ...
  m_pipeline =
      std::make_unique<IndirectPipeline>(IndirectPipeline::Dependencies{
          .renderer = m_renderer,
  ...
  if (m_model) {
    ...
    if (!materials.empty()) {
      m_model->scene().setMaterialBaseIndex(
          m_materialHeap.allocateBlock(materials));
    }
  }
...
void IndirectRenderer::resize(uint32_t width, uint32_t height) {
  auto result =
      m_resourcePool->resize(width, height, static_cast<uint32_t>(r_msaa.get()),
                             r_msaaSampleShading.get());
...
void IndirectRenderer::update(float dt) {
  ...
  if ((m_renderer != nullptr) && (m_renderer->resourceManager() != nullptr)) {
  ...
  if (m_model) {
    ...
    if (!m_model->skins().empty()) {
      ...
      auto skinView = m_model->scene()
                          .registry()
                          .view<SkinComponent, LocalBounds, WorldTransform>();

      std::vector<ecs::Entity> entities;
      skinView.each([&](ecs::Entity entity, SkinComponent &, LocalBounds &,
                        WorldTransform &) { entities.push_back(entity); });

      std::vector<uint8_t> needsUpdate(entities.size(), 0);

      core::TaskSystem::parallelFor(
          (uint32_t)entities.size(),
          [&](enki::TaskSetPartition range, uint32_t) {
            for (uint32_t i = range.start; i < range.end; ++i) {
              ecs::Entity entity = entities[i];
              auto &lb = m_model->scene().registry().get<LocalBounds>(entity);
              auto &wt =
                  m_model->scene().registry().get<WorldTransform>(entity);
      ...
      for (size_t i = 0; i < entities.size(); ++i) {
        if (needsUpdate[i] &&
            !m_model->scene().registry().has<BoundsDirtyTag>(entities[i])) {
          m_model->scene().registry().emplace<BoundsDirtyTag>(entities[i]);
        }
      }
    }
...
TransientAllocation IndirectRenderer::updateGlobalTransforms() {
  ...
  if (alloc.mappedPtr != nullptr) {
    ...
    if (m_model->vertexBuffer() != INVALID_BUFFER_HANDLE) {
      vertexBufferAddr =
          m_renderer->getBuffer(m_model->vertexBuffer())->getDeviceAddress();
    }
    ...
    const auto &topoOrder = m_model->scene().topoOrder();
    core::TaskSystem::parallelFor(
        (uint32_t)topoOrder.size(),
        [&](enki::TaskSetPartition range, uint32_t) {
          for (uint32_t i = range.start; i < range.end; ++i) {
            ecs::Entity e = topoOrder[i];
            auto &inst = dst[e];
...
void IndirectRenderer::processCompletedTextures() {
  ...
  for (const auto handle : completed) {
    if (handle == m_resources.irradianceMap ||
        handle == m_resources.prefilterMap || handle == m_resources.brdfLut ||
  ...
  if (envNeedsUpdate) {
    if (m_sourceSkyboxHandle != INVALID_TEXTURE_HANDLE) {
      Logger::Render.info(
          "Skybox source texture finished loading, re-triggering setSkybox");
      setSkybox(m_sourceSkyboxHandle, m_skyboxFlipY);
    } else {
      Logger::Render.info(
          "IBL/BRDF textures finished loading, re-packing environment data");
      uploadEnvironmentData();
    }
  }
}
...
void IndirectRenderer::updateLightsAndShadows(IndirectDrawContext &ctx) {
  ...
  if (alloc.mappedPtr != nullptr) {
    auto *shadowDataArray =
        reinterpret_cast<::gpu::ShadowDataGPU *>(alloc.mappedPtr);
    ...
    for (uint32_t i = 0; i < shadowDataCount; ++i) {
      ...
      shadowDataArray[i].shadowMapTexture =
          util::u32(rhi::TextureBindlessHandle::Invalid);
      shadowDataArray[i].shadowMapSampler = 0;
    }

    if (m_shadowPassPtr != nullptr && m_resources.shadowCasterIndex >= 0 &&
      ...
      if (tex != nullptr) {
        ...
        entry.shadowMapTexture =
            m_shadowPassPtr->getShadowMapBindlessHandle().index();
        entry.shadowMapSampler = (uint32_t)m_renderer->getBindlessSamplerIndex(
            rhi::SamplerAddressMode::ClampToBorder);
        entry.shadowMapTexelSize = {1.0F / (float)tex->extent().width,
                                    1.0F / (float)tex->extent().height};
      }
    }
  }
}
...
void IndirectRenderer::uploadEnvironmentData() {
  m_sceneUniforms->updateEnvironmentData(
      m_resources.prefilterMap, m_resources.irradianceMap, m_resources.brdfLut,
      m_settings.iblStrength, m_settings.skyboxRotation);
}

void IndirectRenderer::updateMorphTargets(rhi::RHICommandList *) {
...
void IndirectRenderer::buildDrawLists(IndirectDrawContext &ctx,
  ...
  ctx.dodContext.ignoreVisibility =
      (m_settings.cullingMode == CullingMode::GPU);
  ...
  ctx.shadowDodContext.systemMeshCount =
      static_cast<uint32_t>(SystemMeshType::Count);
  ctx.shadowDodContext.ignoreVisibility =
      true; // Shadows should always draw everything

  auto &frame = m_frameManager.getCurrentFrameBuffers();
  if (frame.skinnedVertexBuffer.isValid()) {
    ctx.dodContext.vertexBufferOverride =
        m_renderer->getBuffer(frame.skinnedVertexBuffer.handle())
            ->getDeviceAddress();
    ctx.shadowDodContext.vertexBufferOverride =
        ctx.dodContext.vertexBufferOverride;
  }
  ...
  scene::GLTFUnifiedDOD::buildDrawLists(ctx.dodContext, camera.position(),
                                        cpuTempAllocator);
  scene::GLTFUnifiedDOD::buildDrawLists(ctx.shadowDodContext, camera.position(),
                                        cpuTempAllocator);

  if (ctx.dodContext.transformCount > 0) {
    const size_t bytes =
        ctx.dodContext.transformCount * sizeof(gpu::InstanceData);
    auto alloc = m_frameManager.allocateUpload(bytes, 16);
    if (alloc.mappedPtr != nullptr) {
...
void IndirectRenderer::setSkybox(TextureHandle skybox, bool flipY) {
  ...
  if (m_convertedSkyboxHandle != INVALID_TEXTURE_HANDLE &&
      m_convertedSkyboxHandle != skybox) {
  ...
  if (tex->type() == rhi::TextureType::Texture2D) {
    Logger::Render.info("IndirectRenderer: Converting Equirectangular Skybox "
                        "to Cubemap (1024x1024). Handle: {}",
                        util::u32(skybox.index));
    m_convertedSkyboxHandle =
        processor.convertEquirectangularToCubemap(skybox, 1024);
    ...
    Logger::Render.error("IndirectRenderer: Skybox texture is not a compatible "
                         "type (2D or Cube). Type={}",
                         (int)tex->type());
    return;
  }
...
void IndirectRenderer::loadEnvironmentMap(const std::filesystem::path &path,
                                          bool flipY) {
  if (m_renderer == nullptr) {
    return;
  }
...
void IndirectRenderer::dispatchSkinning(rhi::RHICommandList *cmd) { (void)cmd; }

void IndirectRenderer::calculateFrustumPlanes(const glm::mat4 &viewProj,
...
IndirectDrawContext
IndirectRenderer::prepareFrame(rhi::RHICommandList *cmd,
                               const scene::Camera &camera, uint32_t width,
  ...
  if (!m_model->skins().empty()) {
    ...
    if (!joints.empty()) {
      ...
      if (!m_model->skins().empty()) {
        ...
        if (!skin.joints.empty()) {
          ...
          if (m_model->scene().registry().has<WorldTransform>(j0)) {
            const auto &worldM =
                m_model->scene().registry().get<WorldTransform>(j0).matrix;
            const auto &ibm = skin.inverseBindMatrices[0];
            Logger::Render.debug(
                "Anim: {} | Time: {:.3f} | World0[0,0]: {:.3f} | IBM0[0,0]: "
                "{:.3f} | Final0[0,0]: {:.3f}",
                state.animIndex, state.currentTime, worldM[0][0], ibm[0][0],
                m0[0][0]);
          }
        }
      }
      ...
      frame.jointMatricesBuffer =
          BufferPtr(nullptr, m_jointBuffer.getBufferHandle());
    }
  }
  ...
  if (!m_model->skins().empty() || !m_model->morphStates().empty()) {
    {
      ...
      view.each([&](ecs::Entity, const MeshRenderer &mr,
      ...
      const auto xformAlloc = m_frameManager.allocateUpload(
          meshXforms.size() * sizeof(gpu::MeshXform), 16);
    ...
    if (!frame.skinnedVertexBuffer.isValid() ||
        m_renderer->getBuffer(frame.skinnedVertexBuffer.handle())->size() <
            m_renderer->getBuffer(m_model->vertexBuffer())->size()) {
  ...
  m_sceneUniforms->updateCamera(camera, width, height, m_dt,
                                m_settings.debugLightView, m_shadowPassPtr,
                                ctx.cameraDataSlice, ctx.cameraDataAddr);
  ...
  if (m_settings.cullingMode == CullingMode::CPU) {
    ...
    auto cullView = m_model->scene()
                        .registry()
                        .view<MeshRenderer, WorldBounds, Visibility>();

    std::vector<ecs::Entity> cullEntities;
    for (ecs::Entity entity : cullView) {
      cullEntities.push_back(entity);
    }

    std::atomic<uint32_t> visibleCount{0};
    core::TaskSystem::parallelFor(
        static_cast<uint32_t>(cullEntities.size()),
        [&](enki::TaskSetPartition range, uint32_t) {
          for (uint32_t i = range.start; i < range.end; ++i) {
            ecs::Entity entity = cullEntities[i];
            const auto &wb =
                m_model->scene().registry().get<WorldBounds>(entity);
            auto &vis = m_model->scene().registry().get<Visibility>(entity);

            bool visible = geometry::isBoxInFrustum(frustum, wb.aabb);
            vis.visible = visible ? 1 : 0;
            if (visible) {
              visibleCount.fetch_add(1, std::memory_order_relaxed);
            }
          }
        },
        1024);

    m_visibleMeshCount = visibleCount.load(std::memory_order_relaxed);

    if (m_settings.drawDebugBounds && debugLayer) {
      for (ecs::Entity entity : cullEntities) {
        const auto &wb = m_model->scene().registry().get<WorldBounds>(entity);
        const auto &vis = m_model->scene().registry().get<Visibility>(entity);
        glm::vec3 color = (vis.visible != 0) ? glm::vec3(0.0F, 1.0F, 0.0F)
    ...
    if (m_settings.drawDebugBounds && debugLayer) {
      ...
      for (ecs::Entity entity : cullEntities) {
        const auto &mr = m_model->scene().registry().get<MeshRenderer>(entity);
        const auto &wb = m_model->scene().registry().get<WorldBounds>(entity);
      ...
      if (sceneAABB.isValid()) {
        debugLayer->box(sceneAABB.m_min, sceneAABB.m_max,
                        glm::vec3(0.8f, 0.0f, 0.8f)); // Purple
      }
    }
  } else if (m_settings.cullingMode == CullingMode::None) {
    auto visView = m_model->scene().registry().view<Visibility>();
    visView.each([](ecs::Entity, Visibility &vis) { vis.visible = 1; });
  }
    ...
    if (alloc.mappedPtr) {
      ...
      if (count > 0 && commands) {
        const size_t cmdBytes =
            static_cast<size_t>(count) * sizeof(DrawIndexedIndirectCommandGPU);
        std::memcpy(reinterpret_cast<uint8_t *>(alloc.mappedPtr) + 16, commands,
                    cmdBytes);
      }
    }
    return alloc;
  ...
  frame.indirectOpaqueAlloc =
      uploadIndirect(ctx.dodContext.indirectOpaque, ctx.dodContext.opaqueCount);
  frame.indirectOpaqueDoubleSidedAlloc =
      uploadIndirect(ctx.dodContext.indirectOpaqueDoubleSided,
                     ctx.dodContext.opaqueDoubleSidedCount);
  frame.indirectTransmissionAlloc = uploadIndirect(
      ctx.dodContext.indirectTransmission, ctx.dodContext.transmissionCount);
  frame.indirectTransmissionDoubleSidedAlloc =
      uploadIndirect(ctx.dodContext.indirectTransmissionDoubleSided,
                     ctx.dodContext.transmissionDoubleSidedCount);
  frame.indirectTransparentAlloc = uploadIndirect(
      ctx.dodContext.indirectTransparent, ctx.dodContext.transparentCount);

  frame.indirectOpaqueBuffer = makeSlice(
      *m_renderer, frame.indirectOpaqueAlloc.buffer,
      frame.indirectOpaqueAlloc.offset, frame.indirectOpaqueAlloc.size, 16);
  frame.indirectOpaqueDoubleSidedBuffer =
      makeSlice(*m_renderer, frame.indirectOpaqueDoubleSidedAlloc.buffer,
                frame.indirectOpaqueDoubleSidedAlloc.offset,
                frame.indirectOpaqueDoubleSidedAlloc.size, 16);
  frame.indirectTransmissionBuffer =
      makeSlice(*m_renderer, frame.indirectTransmissionAlloc.buffer,
                frame.indirectTransmissionAlloc.offset,
                frame.indirectTransmissionAlloc.size, 16);
  frame.indirectTransmissionDoubleSidedBuffer =
      makeSlice(*m_renderer, frame.indirectTransmissionDoubleSidedAlloc.buffer,
                frame.indirectTransmissionDoubleSidedAlloc.offset,
                frame.indirectTransmissionDoubleSidedAlloc.size, 16);
  frame.indirectTransparentBuffer =
      makeSlice(*m_renderer, frame.indirectTransparentAlloc.buffer,
                frame.indirectTransparentAlloc.offset,
                frame.indirectTransparentAlloc.size, 16);

  frame.shadowIndirectOpaqueAlloc = uploadIndirect(
      ctx.shadowDodContext.indirectOpaque, ctx.shadowDodContext.opaqueCount);
  frame.shadowIndirectOpaqueDoubleSidedAlloc =
      uploadIndirect(ctx.shadowDodContext.indirectOpaqueDoubleSided,
                     ctx.shadowDodContext.opaqueDoubleSidedCount);

  frame.shadowIndirectOpaqueBuffer =
      makeSlice(*m_renderer, frame.shadowIndirectOpaqueAlloc.buffer,
                frame.shadowIndirectOpaqueAlloc.offset,
                frame.shadowIndirectOpaqueAlloc.size, 16);
  frame.shadowIndirectOpaqueDoubleSidedBuffer =
      makeSlice(*m_renderer, frame.shadowIndirectOpaqueDoubleSidedAlloc.buffer,
...
void IndirectRenderer::draw(
    rhi::RHICommandList *cmd, const Camera &camera, uint32_t width,
    uint32_t height, debug::DebugLayer *debugLayer,
...

  {
    ...
    cmd->pipelineBarrier(rhi::ShaderStage::None, rhi::ShaderStage::RenderTarget,
                         barrier);
  }

  if (auto *profiler = m_renderer->device()->gpuProfiler()) {
    if (auto *assets = m_renderer->assets()) {
      profiler->updateStreamingStatistics(rhiFrameIndex,
                                          assets->getStreamingStatistics());
    }
  }
  ...
  IndirectDrawContext drawCtx =
      prepareFrame(cmd, *activeCamera, width, height, debugLayer);
  ...
  if (m_transmissionPassPtr != nullptr) {
    m_resources.transmissionTexture = m_transmissionPassPtr->getTextureHandle();
  }
  ...
  passCtx.resources.shadowIndirectOpaqueBuffer =
      passCtx.frameBuffers.shadowIndirectOpaqueBuffer;
  passCtx.resources.shadowIndirectOpaqueDoubleSidedBuffer =
      passCtx.frameBuffers.shadowIndirectOpaqueDoubleSidedBuffer;
