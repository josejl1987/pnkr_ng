...
TextureStreamer::BlockInfo
TextureStreamer::getFormatBlockInfo(rhi::Format format) {
  uint32_t w;
  uint32_t h;
  uint32_t b;
...
TextureLoadResult
TextureStreamer::loadTexture(const std::string &path, bool srgb,
                             [[maybe_unused]] uint32_t baseMip) {
  ...
  if (isKTXExtension(path)) {
    ...
    if (headerOnly) {
      ...
      core::Logger::Asset.debug(
          "Small asset (or non-ktx2), forcing full load: {}", path);
    }
    ...
    if (KTXUtils::loadFromFile(path, result.textureData, &err, headerOnly)) {
      ...
      if (needsTranscode && headerOnly) {
        core::Logger::Asset.warn(
            "Asset '{}' is compressed/supercompressed (Scheme: {}). Streaming "
            "disabled. Falling back.",
            path,
            static_cast<uint32_t>(((ktxTexture2 *)result.textureData.texture)
                                      ->supercompressionScheme));
        KTXUtils::destroy(result.textureData);
        if (KTXUtils::loadFromFile(path, result.textureData, &err, false)) {
          result.isRawImage = false;
          result.totalSize = result.textureData.dataSize > 0
                                 ? result.textureData.dataSize
                                 : result.textureData.ownedData.size();
          result.success = true;
          ...
          core::Logger::Asset.error(
              "AsyncLoader: KTX Full Load Retry failed: {}", err);
        }
      } else {
        result.isRawImage = false;
        ...
        if (result.textureData.dataSize > 0) {
          ...
          for (uint32_t level = 0; level < result.textureData.mipLevels;
               ++level) {
            ktx_size_t levelSize =
                ktxTexture_GetImageSize(result.textureData.texture, level);
            estimatedSize += levelSize * result.textureData.numLayers *
    ...
    if (raw != nullptr) {
      ...
      result.textureData.extent = {
          .width = (uint32_t)w, .height = (uint32_t)h, .depth = 1};
      result.textureData.format =
          srgb ? rhi::Format::R8G8B8A8_SRGB : rhi::Format::R8G8B8A8_UNORM;
...
std::optional<CopyRegionPlan> TextureStreamer::planNextCopy(
    const KTXTextureData &textureData, const StreamRequestState &state,
    bool isRawImage, uint64_t stagingCapacity, uint64_t currentStagingOffset,
    rhi::Format format) {
  ...
  if (state.direction == UploadDirection::LowToHighRes) {
    ...
    if (std::cmp_greater_equal(state.currentLevel, effectiveMipLevels)) {
      return std::nullopt;
    }
  }
  ...
  const uint32_t mipW = std::max(1U, textureData.extent.width >> sourceLevel);
  const uint32_t mipH = std::max(1U, textureData.extent.height >> sourceLevel);
  const uint32_t mipD = std::max(1U, textureData.extent.depth >> sourceLevel);

  const uint32_t widthBlocks = (mipW + blk.m_width - 1) / blk.m_width;
  const uint32_t heightBlocks = (mipH + blk.m_height - 1) / blk.m_height;
  ...
  if (isRawImage) {
    ...
    ktxTexture_GetImageOffset(textureData.texture, sourceLevel,
                              state.currentLayer, state.currentFace, &offset);
    memoryPtr = textureData.dataPtr + offset;
  } else if (textureData.mappedFile && textureData.mappedFile->isValid()) {
    fileOff = KTXUtils::getImageFileOffset(
        textureData, sourceLevel, state.currentLayer, state.currentFace);
    if (fileOff > 0) {
      memoryPtr = textureData.mappedFile->data() + fileOff;
    }
  } else {
  ...
  plan.m_region.textureOffset = {
      .x = 0, .y = static_cast<int32_t>(startBlockRow * blk.m_height), .z = 0};
  ...
  plan.m_isMipFinished = (bytesAlreadyCopied + copyBytes >= bytesTotalMip);

  if (isRawImage) {
    core::Logger::Asset.info(
        "TextureStreamer: Copy raw: {}/{} bytes. rows={}, finished={}. "
        "reqLevel={}, mipH={}",
        bytesAlreadyCopied + copyBytes, bytesTotalMip, plan.m_rowsCopied,
        plan.m_isMipFinished, state.currentLevel, mipH);
  }

  return plan;
}

void TextureStreamer::advanceRequestState(StreamRequestState &state,
...
int32_t TextureStreamer::getInitialMipLevel(const KTXTextureData &textureData,
                                            uint32_t baseMip,
                                            UploadDirection direction) {
  uint32_t effectiveMipLevels = std::max(1U, textureData.mipLevels - baseMip);
