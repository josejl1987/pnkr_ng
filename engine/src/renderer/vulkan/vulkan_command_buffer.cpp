...
namespace pnkr::renderer
{
  ...
  {
    if (!m_device) throw std::runtime_error("[VulkanCommandBuffer] device is null");
    if (m_frames == 0) throw std::runtime_error("[VulkanCommandBuffer] framesInFlight must be > 0");

    createPool(graphicsQueueFamilyIndex);
    allocateBuffers();

    pnkr::core::Logger::info("[VulkanCommandBuffer] Created (framesInFlight={})", m_frames);
  }


  VulkanCommandBuffer::VulkanCommandBuffer(VulkanDevice& dev)
  : VulkanCommandBuffer(dev.device(), dev.graphicsQueueFamily(), dev.framesInFlight())
  {}
  ...
  void VulkanCommandBuffer::createPool(uint32_t graphicsQueueFamilyIndex)
  {
    ...
    // Allow resetting individual command buffers (optional), but we’ll reset pool each frame anyway.
    cpci.flags = vk::CommandPoolCreateFlagBits::eResetCommandBuffer;

    m_pool = m_device.createCommandPool(cpci);
    if (!m_pool) throw std::runtime_error("[VulkanCommandBuffer] createCommandPool failed");
  }
  ...
  void VulkanCommandBuffer::allocateBuffers()
  {
    ...
    if (m_cmd.size() != m_frames)
      throw std::runtime_error("[VulkanCommandBuffer] allocateCommandBuffers returned wrong count");
  ...
  {
    if (frame >= m_frames) throw std::runtime_error("[VulkanCommandBuffer] frame index out of range");
  ...
  void VulkanCommandBuffer::submit(uint32_t frame,
