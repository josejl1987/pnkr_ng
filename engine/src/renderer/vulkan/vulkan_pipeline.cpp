...
namespace pnkr::renderer
{
    namespace
    {
        std::vector<std::uint32_t> ReadSpirvU32(const char* path)
        {
            ...
            if (!file.is_open())
            {
                pnkr::core::Logger::error("[VulkanPipeline] Failed to open SPIR-V file: {}", path);
            ...
            {
                pnkr::core::Logger::error("[VulkanPipeline] Invalid SPIR-V size ({} bytes): {}", byteSize, path);
            ...
            if (!file)
            {
                pnkr::core::Logger::error("[VulkanPipeline] Failed to read SPIR-V file: {}", path);
        ...
        vk::ShaderModule CreateShaderModule(vk::Device device, const char* path)
        {
            ...
            {
                pnkr::core::Logger::error("[VulkanPipeline] createShaderModule failed for {}: {}", path, e.what());
                throw;
            }
            return module;
        }
          ...
          , m_colorFormat(colorFormat)
    {
        ...
        m_vertexInput = config.vertexInput;
    ...
    void pnkr::renderer::VulkanPipeline::createPipelineLayout()
    {
        ...
        {
            pnkr::core::Logger::error("[VulkanPipeline] createPipelineLayout failed: {}", e.what());
            throw;
        }
    }

    void pnkr::renderer::VulkanPipeline::createGraphicsPipeline(const PipelineConfig  & config)
    {
        // Shader stages
        ...
        // NOTE: Vulkan screen-space Y is inverted vs OpenGL; your shader can handle it.
        // If your triangle appears mirrored, flip this to eCounterClockwise or adjust shader positions.
        ...
        depthStencil.depthTestEnable = config.enableDepth;
        depthStencil.stencilTestEnable = VK_FALSE;
        depthStencil.depthWriteEnable = config.enableDepth;
  ...
  (m_config.enableDepth ? m_config.depthFormat : vk::Format::eUndefined);
        ...
        vertexInput.pVertexAttributeDescriptions =
            m_vertexInput.attributes.empty() ? nullptr : m_vertexInput.attributes.data();
        ...
        try
        {
            ...
            {
                pnkr::core::Logger::error("[VulkanPipeline] createGraphicsPipeline failed: {}",
        ...
        {
            pnkr::core::Logger::error("[VulkanPipeline] createGraphicsPipeline threw: {}", e.what());
            throw;
        }
    }
}
