...
namespace pnkr::renderer
{
  ...
  void VulkanPipeline::reset() noexcept
  {
    ...
    if (m_layout)   { m_device.destroyPipelineLayout(m_layout); m_layout = nullptr; }
    if (m_frag)     { m_device.destroyShaderModule(m_frag); m_frag = nullptr; }
    if (m_vert)     { m_device.destroyShaderModule(m_vert); m_vert = nullptr; }
  }
  ...
  void pnkr::renderer::VulkanPipeline::createPipelineLayout()
  {
    ...
    // No descriptor sets, no push constants in Stage 4
    try
    {
