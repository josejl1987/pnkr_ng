...
void logOpenCLStatus() {
  std::call_once(sInitFlag, []() {
    ...
    core::Logger::Asset.info("KTXUtils: libktx built WITH OpenCL support");
#else
...
constexpr std::array K_GL_FORMAT_TABLE = {
    ...
    FormatMapping{.m_source = 0x8815, .m_rhi = rhi::Format::R32G32B32_SFLOAT},
...
constexpr std::array K_VK_FORMAT_TABLE = {
    ...
    FormatMapping{.m_source = 97, .m_rhi = rhi::Format::R16G16B16A16_SFLOAT},
    ...
    FormatMapping{.m_source = 109, .m_rhi = rhi::Format::R32G32B32A32_SFLOAT},
    ...
    FormatMapping{.m_source = 130, .m_rhi = rhi::Format::D32_SFLOAT_S8_UINT},
    ...
    FormatMapping{.m_source = 151, .m_rhi = rhi::Format::ETC2_R8G8B8A8_UNORM},
    FormatMapping{.m_source = 152, .m_rhi = rhi::Format::ETC2_R8G8B8A8_SRGB},
};

template <size_t N>
rhi::Format lookupFormat(const std::array<FormatMapping, N> &table,
                         uint32_t source) {
...
bool loadFromTexture(ktxTexture *texture, KTXTextureData &out,
  ...
  out.extent = rhi::Extent3D{.width = texture->baseWidth,
  ...
  if (texture->classId == ktxTexture2_c) {
    ...
    const bool needsTranscode =
        (ktxTexture2_NeedsTranscoding(texture2) == KTX_TRUE);
    ...
    if (needsTranscode) {
      const auto result =
          ktxTexture2_TranscodeBasis(texture2, KTX_TTF_BC7_RGBA, 0);
  ...
  if (texture->classId == ktxTexture2_c && !label.empty()) {
    ...
    if (file.is_open()) {
      ...
      if (sanitizedLevelCount == std::max(1U, texture->numLevels)) {
        ...
        core::Logger::Asset.trace("KTX2 Partial I/O: Parsed {} levels for '{}'",
      ...
      core::Logger::Asset.error(
          "KTX2 Partial I/O: Failed to open file for indexing '{}'", label);
    }
  }
...
bool KTXUtils::loadFromFile(const std::filesystem::path &path,
                            KTXTextureData &out, std::string *error,
                            bool headerOnly) {
  out = {};
  ...
  const auto result =
      ktxTexture_CreateFromNamedFile(pathString.c_str(), flags, &texture);
...
bool KTXUtils::loadFromMemory(std::span<const std::byte> data,
  ...
  const auto result = ktxTexture_CreateFromMemory(
      reinterpret_cast<const ktx_uint8_t *>(data.data()), data.size(),
...
bool KTXUtils::saveToFile(const std::filesystem::path &path,
                          ktxTexture2 *texture, std::string *error) {
  const auto result = ktxTexture_WriteToNamedFile(
      reinterpret_cast<ktxTexture *>(texture), path.string().c_str());
...
ktxTexture2 *KTXUtils::createKTX2Texture(std::span<const std::byte> pixels,
  ...
  const auto result = ktxTexture_SetImageFromMemory(
      reinterpret_cast<ktxTexture *>(texture), 0, 0, 0,
      reinterpret_cast<const ktx_uint8_t *>(pixels.data()), pixels.size());
...
uint64_t KTXUtils::getImageFileOffset(const KTXTextureData &data,
  ...
  if (data.texture->classId == ktxTexture2_c) {
    ...
    uint64_t imageIndex = (layer * (numFaces * numDepth)) + (face * numDepth);
...
KTXTextureData::~KTXTextureData() { KTXUtils::destroy(*this); }
