...
double Application::getRefreshRate() const {
  ...
  return (mode->refresh_rate > 0.0F) ? static_cast<double>(mode->refresh_rate)
                                     : 60.0;
}
...
Application::Application(ApplicationConfig cfg)
    : m_config(std::move(cfg)), m_window(m_config.title, m_config.width,
      ...
      m_baseDir(resolveBasePath()), m_shaderDir(m_baseDir / "shaders") {}
    ...
    m_renderer->setRecordFunc([this](const renderer::RHIFrameContext &ctx) {
      ...
      if (m_imgui.isInitialized()) {
        if (m_renderer->useDefaultRenderPass()) {
          ...
          layoutBarrier.oldLayout =
              renderer::rhi::ResourceLayout::ColorAttachment;
          layoutBarrier.newLayout =
              renderer::rhi::ResourceLayout::ColorAttachment;
          layoutBarrier.srcAccessStage =
              renderer::rhi::ShaderStage::RenderTarget;
          layoutBarrier.dstAccessStage =
              renderer::rhi::ShaderStage::RenderTarget;
          ctx.commandBuffer->pipelineBarrier(
              renderer::rhi::ShaderStage::RenderTarget,
              renderer::rhi::ShaderStage::RenderTarget, layoutBarrier);
...
int Application::run() {
  ...
  try {
    ...
    std::filesystem::path sourceShaders =
        currentPath / "engine/src/renderer/shaders";
    std::filesystem::path sourceAssets =
        currentPath / "samples/rhiIndirectGLTF/assets";
    std::filesystem::path sourceInclude = currentPath / "engine/include";

    for (int depth = 0; depth < 5; ++depth) {
      if (std::filesystem::exists(currentPath /
                                  "engine/src/renderer/shaders")) {
    ...
    if (m_config.createRenderer) {
      m_renderer = std::make_unique<renderer::RHIRenderer>(
          m_window, m_config.rendererConfig);
    ...
    while (m_window.isRunning()) {
      ...
      if (m_renderer && m_imgui.isInitialized()) {
        ...
        if (ImGui::Begin("Settings")) {
          ...
          const char *items[] = {"Trace", "Debug",    "Info", "Warn",
                                 "Error", "Critical", "Off"};
          if (ImGui::Combo("Log Level", &currentItem, items,
                           IM_ARRAYSIZE(items))) {
            pnkr::core::Logger::setLevel(
                static_cast<pnkr::core::LogLevel>(currentItem));
            pnkr::core::Logger::Platform.info("Log level changed to {}",
                                              items[currentItem]);
          }
          ...
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("Deletes the cache file and resets the internal "
                              "Vulkan cache.\nUse this if shaders are not "
                              "updating or the file is too large.");
          }
          ...
          if (ImGui::Button("Wipe Asset Cache")) {
            ...
            pnkr::Log::info(
                "Asset cache wiped. Restart application to regenerate.");
          }
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("Deletes cached .pmesh and texture "
      ...
      std::chrono::duration<float, std::milli> updateMs =
          updateEnd - updateStart;
      std::chrono::duration<float, std::milli> renderMs =
      ...
      if (totalMs > avgFrameTime * 2.0F && totalMs > 20.0F) {
        pnkr::Log::warn("Frame Jitter Detected: {:.2f}ms (Avg: {:.2f}ms) | "
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "PNKR Engine - Fatal Error",
                             e.what(), nullptr);
    e.trace().print();
    return 1;
