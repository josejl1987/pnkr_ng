...
// Null RHI Includes
#include "null/null_device.hpp"
#include "null/null_swapchain.hpp"
    ...
    if (pCallbackData->objectCount > 0) {
      for (uint32_t i = 0; i < pCallbackData->objectCount; i++) {
        ...
        if (devicePtr != nullptr) {
          ...
          if (device->tryGetObjectTrace(static_cast<uint64_t>(obj.objectHandle),
                                        tracked)) {
            ...
            core::Logger::RHI.warn(
                "   [Trace Missing] Object Handle={:#x} Type={}",
                obj.objectHandle, vk::to_string(obj.objectType));
          }
        } else {
          core::Logger::RHI.warn("   [Device Ptr nullptr] Cannot retrieve "
                                 "creation trace for handle {:#x}",
                                 obj.objectHandle);
        }
      }
    }
...
bool hasExtension(const std::vector<vk::ExtensionProperties> &exts,
                  const char *name) {
...
RHIFactory::enumeratePhysicalDevices(RHIBackend backend) {
    ...
    try {
      ...
      if (vkGetInstanceProcAddr == nullptr) {
        throw cpptrace::runtime_error(
            "Failed to load vkGetInstanceProcAddr from Vulkan loader");
      }
      ...
      if (enableValidation) {
        core::Logger::RHI.info(
            "Attempting to enable validation layers (Forced)...");
      }

      if (enableValidation &&
          hasLayer(availableLayers, "VK_LAYER_KHRONOS_validation")) {
        layers.push_back("VK_LAYER_KHRONOS_validation");
      } else if (enableValidation) {
        core::Logger::RHI.warn(
            "Validation layer VK_LAYER_KHRONOS_validation not found in "
            "enumeration. Proceeding with Debug Utils setup.");
      }

      bool enableDebugUtils = false;
      bool hasBdaReportExt = false;
      if (enableValidation && hasExtension(availableExtensions,
        ...
        core::Logger::RHI.warn(
            "VK_EXT_debug_utils not available; debug messenger disabled");
      }
      ...
      if (enableDebugUtils) {
        instanceContext->debugMessenger =
            instanceContext->instance.createDebugUtilsMessengerEXT(
                debugCreateInfo);
        instanceContext->hasDebugMessenger = true;
      }

      auto physicalDevices =
          instanceContext->instance.enumeratePhysicalDevices();

      for (auto pd : physicalDevices) {
        devices.push_back(std::make_unique<vulkan::VulkanRHIPhysicalDevice>(
            pd, instanceContext));
      }

      core::Logger::RHI.info("Found {} Vulkan physical device(s)",
                             devices.size());
  ...
  case RHIBackend::Null: {
    devices.push_back(std::make_unique<NullRHIPhysicalDevice>());
    break;
  }
...
RHIFactory::createDevice(RHIBackend backend,
  ...
  case RHIBackend::Vulkan: {
    ...
    auto *vkPhysicalDevice =
        vulkan::rhi_cast<vulkan::VulkanRHIPhysicalDevice>(physicalDevice.get());
    ...
    std::unique_ptr<vulkan::VulkanRHIPhysicalDevice> ownedPhysicalDevice(
        static_cast<vulkan::VulkanRHIPhysicalDevice *>(
            physicalDevice.release()));

    auto device =
        vulkan::VulkanRHIDevice::create(std::move(ownedPhysicalDevice), desc);
  ...
  case RHIBackend::Null: {
    ...
    auto *nullPhysicalDevice =
        static_cast<NullRHIPhysicalDevice *>(physicalDevice.release());
    return std::make_unique<NullRHIDevice>(
        std::unique_ptr<NullRHIPhysicalDevice>(nullPhysicalDevice));
  }
  case RHIBackend::DirectX12:
    core::Logger::RHI.error("DirectX12 backend not implemented");
    return nullptr;
...
std::unique_ptr<RHIDevice>
RHIFactory::createDeviceAuto(RHIBackend backend, const DeviceDescriptor &desc) {
  ...
  core::Logger::RHI.info("Selected device: {}",
                         bestDevice->capabilities().deviceName);

  return createDevice(backend, std::move(bestDevice), desc);
}

std::unique_ptr<RHISwapchain>
RHIFactory::createSwapchain(RHIDevice *device, platform::Window &window,
  ...
  if (auto *vkDevice = dynamic_cast<vulkan::VulkanRHIDevice *>(device)) {
    try {
      return std::make_unique<vulkan::VulkanRHISwapchain>(vkDevice, window,
                                                          preferredFormat);
    } catch (const std::exception &e) {
      core::Logger::RHI.error("Failed to create Vulkan swapchain: {}",
                              e.what());
      return nullptr;
    }
  } else if (auto *nullDevice = static_cast<NullRHIDevice *>(device)) {
    return std::make_unique<NullRHISwapchain>(nullDevice, preferredFormat);
  }
