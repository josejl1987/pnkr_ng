...
bool VulkanDeletionQueue::shouldTraceObjects() {
  static auto *cvar = static_cast<core::CVar<bool> *>(
      core::CVarSystem::find("r_vkTraceObjects"));

  return cvar ? cvar->get() : false;
}
void VulkanDeletionQueue::enqueue(uint64_t currentFrame,
                                  std::function<void()> &&deleteFn) {
  std::scoped_lock lock(m_deletionMutex);
  m_deletionQueue.push_back(
      {.frameIndex = currentFrame, .deleteFn = std::move(deleteFn)});
}
...
void VulkanDeletionQueue::trackObject(vk::ObjectType type, uint64_t handle,
  ...
  if (VulkanDeletionQueue::shouldTraceObjects()) {
    tracked.trace = cpptrace::generate_trace(2).to_string();
  } else {
    tracked.trace.clear();
  }

  core::Logger::RHI.trace("Tracking Object: Handle={:#x}, Type={}, Name='{}'",
                          handle, vk::to_string(type), name);
...
bool VulkanDeletionQueue::tryGetObjectTrace(uint64_t handle,
