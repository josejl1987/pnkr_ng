...
AUTO_CVAR_BOOL(r_vkTraceObjects,
               "Enable Vulkan object stack trace tracking (performance impact)",
               false, core::CVarFlags::save, [](bool value) {
                 pnkr::core::Logger::RHI.info(
                     "Vulkan object tracing {} (performance {})",
                     value ? "enabled" : "disabled",
                     value ? "degraded" : "optimized");
               });
...
void VulkanRHIPhysicalDevice::queryCapabilities() {
  ...
  // Strict Hardware Capability Check for Bindless Features
  {
    ...
    bool supported = true;
    supported &= checkFeature(indexingFeatures.descriptorBindingPartiallyBound,
                              "descriptorBindingPartiallyBound");
    supported &= checkFeature(indexingFeatures.runtimeDescriptorArray,
                              "runtimeDescriptorArray");
    supported &= checkFeature(
    ...
    supported &=
        checkFeature(indexingFeatures.shaderSampledImageArrayNonUniformIndexing,
                     "shaderSampledImageArrayNonUniformIndexing");

    if (!supported) {
      throw std::runtime_error(
          "GPU does not support required bindless features (Descriptor "
          "Indexing). PNKR Engine require these features to run.");
    }
  }
  ...
  m_capabilities.maxCombinedSampleCount = std::min(
      m_capabilities.maxColorSampleCount, m_capabilities.maxDepthSampleCount);
}
...
void VulkanRHIPhysicalDevice::queryQueueFamilies() {
  ...
  for (const auto &queueFamilyProperty : queueFamilyProps) {
    ...
    info.familyIndex =
        static_cast<uint32_t>(&queueFamilyProperty - &queueFamilyProps[0]);
    info.queueCount = queueFamilyProperty.queueCount;
    info.supportsGraphics = (queueFamilyProperty.queueFlags &
                             vk::QueueFlagBits::eGraphics) != vk::QueueFlags{};
    info.supportsCompute = (queueFamilyProperty.queueFlags &
                            vk::QueueFlagBits::eCompute) != vk::QueueFlags{};
    info.supportsTransfer = (queueFamilyProperty.queueFlags &
...
public:
  ...
  void selectQueues() {
    ...
    PNKR_ASSERT(graphicsFamily != VK_QUEUE_FAMILY_IGNORED,
  ...
  void createLogicalDevice() {
    ...
    featuresCore.pipelineStatisticsQuery =
        m_physicalDevice->capabilities().pipelineStatisticsQuery ? VK_TRUE
                                                                 : VK_FALSE;
    m_enabledFeatures = featuresCore;

    vk::PhysicalDeviceVulkan12Features features12{};
    features12.drawIndirectCount =
        m_physicalDevice->capabilities().drawIndirectCount ? VK_TRUE : VK_FALSE;
    ...
    auto availExts =
        m_physicalDevice->physicalDevice().enumerateDeviceExtensionProperties();
    ...
    if (instanceHasDebugUtils &&
        hasExt("VK_EXT_device_address_binding_report")) {
    ...
    if (enableAftermath &&
        hasExt(VK_NV_DEVICE_DIAGNOSTICS_CONFIG_EXTENSION_NAME) &&
        hasExt("VK_NV_device_diagnostic_checkpoints")) {
      ...
      core::Logger::RHI.warn("NVIDIA Aftermath is enabled in build, but "
                             "required Vulkan extensions are missing: {} or {}",
    ...
    if (hasExt(VK_KHR_RELAXED_BLOCK_LAYOUT_EXTENSION_NAME)) {
      extensions.push_back(VK_KHR_RELAXED_BLOCK_LAYOUT_EXTENSION_NAME);
    }
    ...
    const bool enableMinLodExt =
        hasExt(VK_EXT_IMAGE_VIEW_MIN_LOD_EXTENSION_NAME);
    if (enableMinLodExt) {
  ...
  void initVMA() {
    ...
    funcs.vkFlushMappedMemoryRanges =
        VULKAN_HPP_DEFAULT_DISPATCHER.vkFlushMappedMemoryRanges;
    funcs.vkInvalidateMappedMemoryRanges =
    ...
    funcs.vkGetDeviceBufferMemoryRequirements =
    ...
    auto result =
        static_cast<vk::Result>(vmaCreateAllocator(&allocatorInfo, &allocator));
    if (result != vk::Result::eSuccess) {
      throw cpptrace::runtime_error("Failed to create VMA allocator: " +
                                    vk::to_string(result));
    }
  ...
  void initBindless() {
    m_bindlessManager = std::make_unique<BindlessDescriptorManager>(
        m_device, m_physicalDevice->physicalDevice());
  }
  ...
  void initGpuProfiler() {
    ...
    auto timestampPeriod = static_cast<double>(props.limits.timestampPeriod);
    const auto queueFamilyProps =
        m_physicalDevice->physicalDevice().getQueueFamilyProperties();
    const uint32_t validBits =
        (m_queues.graphicsFamily < queueFamilyProps.size())
            ? queueFamilyProps[m_queues.graphicsFamily].timestampValidBits
            : 0;
    core::Logger::RHI.trace(
        "GPU profiler: timestampPeriod={} ns, timestampValidBits={}",
        timestampPeriod, validBits);
    if (validBits == 0) {
      core::Logger::RHI.warn("GPU profiler: timestamp queries are not "
                             "supported on this queue family");
    }

    m_gpuProfiler->init(
        m_device, 4096, timestampPeriod,
        m_physicalDevice->capabilities().pipelineStatisticsQuery);
  }
  ...
  void initPipelineCache() {
    ...
    if (!m_pipelineCache) {
      core::Logger::RHI.warn(
          "Failed to create pipeline cache, using null cache");
    }
  }

  void initBDARegistry() { m_bdaRegistry = std::make_unique<BDARegistry>(); }
...
std::unique_ptr<VulkanRHIDevice>
VulkanRHIDevice::create(std::unique_ptr<VulkanRHIPhysicalDevice> physicalDevice,
...
VulkanRHIDevice::VulkanRHIDevice(VulkanDeviceConstructionContext &&ctx)
    : m_physicalDevice(std::move(ctx.physicalDevice)), m_device(ctx.device),
      ...
      m_descriptorPool(ctx.descriptorPool), m_pipelineCache(ctx.pipelineCache) {
  if (ctx.calibratedTimestampsEnabled) {
    getPhysicalDeviceCalibrateableTimeDomainsEXT =
        reinterpret_cast<PFN_vkGetPhysicalDeviceCalibrateableTimeDomainsEXT>(
            m_physicalDevice->instance().getProcAddr(
                "vkGetPhysicalDeviceCalibrateableTimeDomainsEXT"));
    getCalibratedTimestampsEXT =
        reinterpret_cast<PFN_vkGetCalibratedTimestampsEXT>(
            m_device.getProcAddr("vkGetCalibratedTimestampsEXT"));

    if (!getPhysicalDeviceCalibrateableTimeDomainsEXT ||
        !getCalibratedTimestampsEXT) {
      core::Logger::RHI.error("Failed to load VK_EXT_calibrated_timestamps "
                              "functions despite extension being enabled.");
      getPhysicalDeviceCalibrateableTimeDomainsEXT = nullptr;
      getCalibratedTimestampsEXT = nullptr;
    } else {
      core::Logger::RHI.info(
          "Calibrated timestamps enabled for Tracy profiling.");
    }
  }

  m_resourceFactory = std::make_unique<VulkanResourceFactory>(*this);
  m_deletionQueueMgr = std::make_unique<VulkanDeletionQueue>();
  m_syncManager = std::make_unique<VulkanSyncManager>(
      *this, ctx.queues.graphics, ctx.queues.compute, ctx.queues.transfer,
      ctx.frameTimelineSemaphore, ctx.computeTimelineSemaphore);
  ...
  trackObject(vk::ObjectType::eSemaphore,
              pnkr::util::u64(static_cast<VkSemaphore>(
                  m_syncManager->frameTimelineSemaphore())),
              "FrameTimelineSemaphore");
  trackObject(vk::ObjectType::eSemaphore,
              pnkr::util::u64(static_cast<VkSemaphore>(
                  m_syncManager->computeTimelineSemaphore())),
  ...
  core::Logger::RHI.info("Vulkan RHI Device created: {}",
                         m_physicalDevice->capabilities().deviceName);
}
...
VulkanRHIDevice::~VulkanRHIDevice() {
  if (m_device) {
    ...
    if (m_commandPool) {
      untrackObject(pnkr::util::u64(static_cast<VkCommandPool>(m_commandPool)));
...
void VulkanRHIDevice::savePipelineCache() {
  size_t cacheSize = 0;
  auto result =
      m_device.getPipelineCacheData(m_pipelineCache, &cacheSize, nullptr);
  if (result == vk::Result::eSuccess && cacheSize > 0) {
    std::vector<uint8_t> cacheData(cacheSize);
    result = m_device.getPipelineCacheData(m_pipelineCache, &cacheSize,
                                           cacheData.data());
    if (result == vk::Result::eSuccess) {
...
VulkanRHIDevice::createBuffer(const char *name, const BufferDescriptor &desc) {
  return m_resourceFactory->createBuffer(name, desc);
}

std::unique_ptr<RHITexture>
VulkanRHIDevice::createTexture(const char *name,
                               const TextureDescriptor &desc) {
  return m_resourceFactory->createTexture(name, desc);
}

std::unique_ptr<RHITexture>
VulkanRHIDevice::createTextureView(const char *name, RHITexture *parent,
                                   const TextureViewDescriptor &desc) {
  return m_resourceFactory->createTextureView(name, parent, desc);
}

std::unique_ptr<RHITexture>
VulkanRHIDevice::createTexture(const Extent3D &extent, Format format,
...
std::unique_ptr<RHITexture>
VulkanRHIDevice::createCubemap(const Extent3D &extent, Format format,
                               TextureUsageFlags usage, uint32_t mipLevels) {
  TextureDescriptor desc{};
  desc.extent = extent;
  desc.format = format;
...
VulkanRHIDevice::createSampler(Filter minFilter, Filter magFilter,
                               SamplerAddressMode addressMode,
                               CompareOp compareOp) {
  return m_resourceFactory->createSampler(minFilter, magFilter, addressMode,
                                          compareOp);
}

std::unique_ptr<RHICommandBuffer>
VulkanRHIDevice::createCommandBuffer(RHICommandPool *pool) {
  return m_resourceFactory->createCommandBuffer(pool);
}

std::unique_ptr<RHICommandPool>
VulkanRHIDevice::createCommandPool(const CommandPoolDescriptor &desc) {
  return m_resourceFactory->createCommandPool(desc);
}

VulkanRHICommandPool::VulkanRHICommandPool(VulkanRHIDevice *device,
                                           const CommandPoolDescriptor &desc)
    : m_device(device), m_queueFamilyIndex(desc.queueFamilyIndex) {
  vk::CommandPoolCreateInfo poolInfo{};
...
void VulkanRHICommandPool::reset() {
  if (m_pool) {
    m_device->device().resetCommandPool(
        m_pool, vk::CommandPoolResetFlagBits::eReleaseResources);
  }
}
...
VulkanRHIDevice::createComputePipeline(const ComputePipelineDescriptor &desc) {
  return m_resourceFactory->createComputePipeline(desc);
}

std::unique_ptr<RHIDescriptorSetLayout>
VulkanRHIDevice::createDescriptorSetLayout(const DescriptorSetLayout &desc) {
  return m_resourceFactory->createDescriptorSetLayout(desc);
}

std::unique_ptr<RHIDescriptorSet>
VulkanRHIDevice::allocateDescriptorSet(RHIDescriptorSetLayout *layout) {
...
void VulkanRHIDevice::waitIdle() { m_syncManager->waitIdle(); }
...
void VulkanRHIDevice::submitCommands(
    ...
    const std::vector<uint64_t> &signalSemaphores, RHISwapchain *swapchain) {
  m_syncManager->submitCommands(commandBuffer, signalFence, waitSemaphores,
                                signalSemaphores, swapchain);
}
...
void VulkanRHIDevice::queueSubmit(vk::Queue queue,
...
void VulkanRHIDevice::immediateSubmit(
    std::function<void(RHICommandList *)> &&func) {
  m_syncManager->immediateSubmit(std::move(func));
}

void VulkanRHIDevice::downloadTexture(RHITexture *texture,
  ...
  immediateSubmit([&](RHICommandList *cmd) {
    ...
    copyRegion.imageExtent.width =
        std::max(1U, copyRegion.imageExtent.width >> subresource.mipLevel);
    copyRegion.imageExtent.height =
        std::max(1U, copyRegion.imageExtent.height >> subresource.mipLevel);

    vkCmd->commandBuffer().copyImageToBuffer(
        vk::Image(vkTex->imageHandle()), vk::ImageLayout::eTransferSrcOptimal,
        rhi_cast<VulkanRHIBuffer>(stagingBuffer.get())->buffer(), copyRegion);
...
vk::ShaderModule
VulkanRHIDevice::createShaderModule(const std::vector<uint32_t> &spirvCode) {
...
public:
  ...
  ~VulkanRHIUploadContext() override {
    m_device->untrackObject(pnkr::util::u64(static_cast<VkFence>(m_fence)));
  ...
  void uploadTexture(RHITexture *texture, std::span<const std::byte> data,
    ...
    vkTex->transitionLayout(
        static_cast<VkImageLayout>(vk::ImageLayout::eTransferDstOptimal),
        m_cmd);
    ...
    m_cmd.copyBufferToImage(
        rhi_cast<VulkanRHIBuffer>(m_stagingBuffer.get())->buffer(),
        vk::Image(vkTex->imageHandle()), vk::ImageLayout::eTransferDstOptimal,
        1, &region);
  ...
  void uploadBuffer(RHIBuffer *buffer, std::span<const std::byte> data,
    ...
    m_cmd.copyBuffer(rhi_cast<VulkanRHIBuffer>(m_stagingBuffer.get())->buffer(),
  ...
  void flush() override {
    ...
    auto result =
        m_device->device().waitForFences(1, &m_fence, VK_TRUE, UINT64_MAX);
    if (result != vk::Result::eSuccess) {
...
VulkanRHIDevice::createUploadContext(uint64_t stagingBufferSize) {
...
void VulkanRHIDevice::clearPipelineCache() {
  ...
  if (!m_pipelineCache) {
    core::Logger::RHI.warn(
        "Failed to recreate pipeline cache, using null cache");
    return;
  }
...
size_t VulkanRHIDevice::getPipelineCacheSize() const {
  ...
  size_t size = 0;
  if (m_device.getPipelineCacheData(m_pipelineCache, &size, nullptr) !=
      vk::Result::eSuccess) {
    return 0;
  }
  return size;
...
void VulkanRHIDevice::enqueueDeletion(std::function<void()> &&deleteFn) {
  m_deletionQueueMgr->enqueue(m_syncManager->getCurrentFrame(),
                              std::move(deleteFn));
}
...
void VulkanRHIDevice::trackObject(vk::ObjectType type, uint64_t handle,
...
bool VulkanRHIDevice::tryGetObjectTrace(uint64_t handle,
                                        TrackedVulkanObject &out) const {
...
void VulkanRHIDevice::auditBDA([[maybe_unused]] uint64_t address,
...
void VulkanRHIDevice::submitComputeCommands(RHICommandList *commandBuffer,
                                            bool waitForPreviousCompute,
                                            bool signalGraphicsQueue) {
  m_syncManager->submitComputeCommands(commandBuffer, waitForPreviousCompute,
                                       signalGraphicsQueue);
}
...
void VulkanRHIDevice::reportGpuFault() {
  ...
  auto getFaultInfo = (PFN_vkGetDeviceFaultInfoEXT)m_device.getProcAddr(
  ...
  VkResult res =
      getFaultInfo(static_cast<VkDevice>(m_device), nullptr, &faultInfo);

  if (res != VK_SUCCESS) {
    core::Logger::RHI.critical(
        "GPU Hang detected, but GetDeviceFaultInfoEXT failed with {}",
        static_cast<int>(res));
    return;
  }

  core::Logger::RHI.critical(
      "=== GPU FAULT DETECTED - VK_ERROR_DEVICE_LOST ===");

  core::Logger::RHI.critical("Description: {}",
  ...
  for (uint32_t i = 0; i < 8; ++i) {
    ...
    core::Logger::RHI.critical("  [{:#x}] Address: {:#x} | Precision: {}", i,
                               faultedAddr,
                               static_cast<int>(addrInfo.addressPrecision));
    ...
    if (m_bdaRegistry) {
      ...
      if (alloc != nullptr) {
        ...
        core::Logger::RHI.critical("    Status: Created at frame {}, Alive: {}",
                                   alloc->frameCreated,
                                   alloc->isAlive ? "YES" : "NO (FREED)");

        if (!alloc->isAlive) {
          core::Logger::RHI.critical("    *** USE-AFTER-FREE DETECTED ***");
  ...
  if (m_bdaRegistry) {
    ...
    for (const auto &range : snapshot) {
      if (range.alive) {
        liveCount++;
        core::Logger::RHI.critical(
            "  Live[{}]: <{:#x} - {:#x}> ({} bytes) | Seq: {}", liveCount,
            range.base, range.base + range.size, range.size, range.sequence);

        for (const auto &obj : range.objects) {
          if (!obj.name.empty()) {
            core::Logger::RHI.critical(
                "    -> Object: '{}' | Type: {} | Handle: {:#x}", obj.name,
                static_cast<int>(obj.type), obj.handle);
          }
        }
      }
    }
