...
{
    BindlessDescriptorManager::BindlessDescriptorManager(vk::Device device, vk::PhysicalDevice physicalDevice)
    ...
    void BindlessDescriptorManager::init(VulkanRHIDevice* rhiDevice)
    {
        ...
        }
        {
             ...
             desc.skipBindless = true;
             m_dummyStorageImage = m_rhiDevice->createTexture(desc.debugName.c_str(),desc);
        }
        {
        ...
        {
            m_dummySampler = m_rhiDevice->createSampler(Filter::Nearest, Filter::Nearest, SamplerAddressMode::ClampToEdge);
        }
        ...
        layoutDesc.bindings.push_back({.binding = 0, .type = DescriptorType::SampledImage, .count = maxSampledImages, .stages = ShaderStage::All, .name = "bindlessTextures"});
        layoutDesc.bindings.push_back({.binding = 1, .type = DescriptorType::Sampler, .count = maxSamplers, .stages = ShaderStage::All, .name = "bindlessSamplers"});
        layoutDesc.bindings.push_back({.binding = 2, .type = DescriptorType::SampledImage, .count = maxSampledImages, .stages = ShaderStage::All, .name = "bindlessCubemaps"});
        layoutDesc.bindings.push_back({.binding = 3, .type = DescriptorType::StorageBuffer, .count = maxStorageBuffers, .stages = ShaderStage::All, .name = "bindlessStorageBuffers"});
        layoutDesc.bindings.push_back({.binding = 4, .type = DescriptorType::StorageImage, .count = maxStorageImages, .stages = ShaderStage::All, .name = "bindlessStorageImages"});
        layoutDesc.bindings.push_back({.binding = 5, .type = DescriptorType::SampledImage, .count = maxSampledImages, .stages = ShaderStage::All, .name = "bindlessTextures3D"});
        layoutDesc.bindings.push_back({.binding = 6, .type = DescriptorType::Sampler, .count = maxSamplers, .stages = ShaderStage::All, .name = "bindlessSamplersShadow"});
        layoutDesc.bindings.push_back({.binding = 7, .type = DescriptorType::SampledImage, .count = maxSampledImages, .stages = ShaderStage::All, .name = "bindlessTexturesShadow"});
        layoutDesc.bindings.push_back({.binding = 8, .type = DescriptorType::SampledImage, .count = maxSampledImages, .stages = ShaderStage::All, .name = "bindlessMSTextures"});

        m_bindlessLayout = std::make_unique<VulkanRHIDescriptorSetLayout>(m_device, vkLayout, layoutDesc);
        ...
        m_bindlessSetWrapper = std::make_unique<VulkanRHIDescriptorSet>(
            m_device,
            m_bindlessLayout.get(),
            m_bindlessSet
        );
    }

    void BindlessDescriptorManager::updateSampler(TextureBindlessHandle imageHandle, RHISampler* sampler)
    {
    ...
    void BindlessDescriptorManager::updateTexture(TextureBindlessHandle handle, RHITexture* texture)
    {
      if (!handle.isValid() || (texture == nullptr)) {
        ...
        if (handle.isValid()) {
             ...
             switch (texture->type()) {
                 case TextureType::TextureCube: m_cubemapManager.markOccupied(handle.index(), info); break;
                 default:
                   if (texture->sampleCount() > 1) {
    ...
    TextureBindlessHandle BindlessDescriptorManager::registerTexture(RHITexture* texture, RHISampler* sampler)
    {
    ...
    TextureBindlessHandle BindlessDescriptorManager::registerCubemap(RHITexture* texture, RHISampler* sampler)
    {
    ...
    void BindlessDescriptorManager::releaseShadowSampler(SamplerBindlessHandle handle) {
      if (!handle.isValid()) {
        return;
      }
        ...
        m_shadowSamplerManager.freeDeferred(handle.index(), m_rhiDevice->getCurrentFrame());
    }

    void BindlessDescriptorManager::releaseStorageImage(TextureBindlessHandle handle) {
      if (!handle.isValid()) {
        return;
      }
        ...
        m_storageImageManager.freeDeferred(handle.index(), m_rhiDevice->getCurrentFrame());
    }
    ...
    void BindlessDescriptorManager::releaseShadowTexture2D(TextureBindlessHandle handle) {
      if (!handle.isValid()) {
        return;
      }
        ...
        m_shadowTextureManager.freeDeferred(handle.index(), m_rhiDevice->getCurrentFrame());
    }

    void BindlessDescriptorManager::releaseMSTexture2D(TextureBindlessHandle handle) {
      if (!handle.isValid()) {
        return;
      }
        ...
        m_msaaTextureManager.freeDeferred(handle.index(), m_rhiDevice->getCurrentFrame());
    }
