...
#include <array>
#include <limits>
#include "pnkr/renderer/scene/VtxData.hpp" // For BoundingBox
...
namespace pnkr::renderer::geometry {
    ...
    // Extract world-space corners of the frustum
    inline void getFrustumCorners(const glm::mat4& vp, glm::vec4* points) {
        const glm::vec4 ndcCorners[8] = {
            {-1, -1, 0, 1}, { 1, -1, 0, 1}, // Near bottom
            { 1,  1, 0, 1}, {-1,  1, 0, 1}, // Near top
            {-1, -1, 1, 1}, { 1, -1, 1, 1}, // Far bottom
            { 1,  1, 1, 1}, {-1,  1, 1, 1}  // Far top
        };
    ...
    // Robust Intersection Test: Box outside Frustum Planes OR Frustum outside Box Planes
    inline bool isBoxInFrustum(const Frustum& f, const scene::BoundingBox& b) {
        // 1. Check Box against Frustum Planes
        for (int i = 0; i < 6; i++) {
            int out = 0;
        ...
        // 2. Check Frustum Corners against Box (reduces false positives for large objects)
        int r = 0;
