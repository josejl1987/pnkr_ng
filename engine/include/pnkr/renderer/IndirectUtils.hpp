...
namespace pnkr::renderer {
    ...
    using IndirectCommand = gpu::DrawIndexedIndirectCommandGPU;
    ...
    public:
            ...
            : m_renderer(renderer), m_maxCommands(maxCommands)
        {
            ...
            for (uint32_t i = 0; i < maxFramesInFlight; ++i) {
                ...
                m_buffers[i] = m_renderer->createBuffer(m_bufferNames.back().c_str(), {
                    .size = size,
            ...
            for(uint32_t i = 0; i < maxFramesInFlight; ++i) {
                ...
                m_stagingBuffers[i] = m_renderer->createBuffer(m_stagingNames.back().c_str(), {
                    .size = size,
        ...
        void addCommands(const std::vector<IndirectCommand>& cmds) {
            addCommands(std::span<const IndirectCommand>(cmds.data(), cmds.size()));
        }

        void addCommands(std::span<const IndirectCommand> cmds) {
            size_t toCopy = std::min(cmds.size(), (size_t)(m_maxCommands - m_commands.size()));
            if (toCopy > 0) {
                m_commands.insert(m_commands.end(), cmds.begin(), cmds.begin() + toCopy);
            }
        }

        void upload(rhi::RHICommandList* cmd, uint32_t frameIndex) {
