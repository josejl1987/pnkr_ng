...
enum class RHIBackend { Vulkan, DirectX12, Metal, Null, Auto };
...
struct TextureCopyRegion {
  ...
  // For vkCmdBlitImage: These define the corners of the source and destination
  // regions. For vkCmdCopyImage/vkCmdResolveImage: These are typically {0,0,0}
  // for full subresource operations.
  Offset3D srcOffsets[2];
  Offset3D dstOffsets[2];

  // Compatibility with existing code that expects srcOffset/dstOffset as single
  // points (index 0) We can add accessors or just use srcOffsets[0] in
  // implementation. However, to match previous struct structure closer for code
  // compatibility:
...
enum class MemoryUsage { GPUOnly, CPUToGPU, GPUToCPU, CPUOnly, GPULazy };
...
inline const BindlessHandle<Tag> BindlessHandle<Tag>::Invalid =
    BindlessHandle<Tag>(kInvalidBindlessIndex);
...
static_assert(((uint32_t)ShaderStage::Vertex &
               (uint32_t)ShaderStage::Compute) == 0);
static_assert(((uint32_t)ShaderStage::Fragment &
               (uint32_t)ShaderStage::Compute) == 0);
static_assert(((uint32_t)ShaderStage::Compute &
               (uint32_t)ShaderStage::TessControl) == 0);
static_assert(((uint32_t)ShaderStage::Compute &
               (uint32_t)ShaderStage::TessEval) == 0);
static_assert(((uint32_t)ShaderStage::Vertex &
               (uint32_t)ShaderStage::TessControl) == 0);
} // namespace pnkr::renderer::rhi
